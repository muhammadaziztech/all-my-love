...
class CartItems extends HTMLElement {
  ...
  updateQuantity(line, quantity, name, variantId) {
    this.enableLoading(line);
      ...
      try {
            const cartItem = document.querySelector(`#CartItem-${line}`)

            if(cartItem){
               const { productId, variantId, preorderType } = cartItem.dataset;
               const quantityElement = cartItem.querySelector('input[data-default-value]')

               if(quantityElement){
                   const defaultQuantity = quantityElement.getAttribute('data-default-value')
                 
                   if(defaultQuantity && productId && variantId && preorderType){
                       if(window?.Globo?.Preorder && window?.GloboPreorderParams?.products){
                           let product = window.GloboPreorderParams.products.find(product => product.id.toString() == productId)
                           ...
                           if(product){
                               ...
                               if(profile?.isPreorder){
                                   ...
                                   if(preorderType == 'preorder' && parseInt(moreQuantity) > 0){
                                       alert(`The number of products instock has reached the limit`)
                                       quantity = defaultQuantity;
                                       quantityElement.setAttribute('data-default-value', defaultQuantity)
                                   }
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        document.dispatchEvent(new CustomEvent('globo.preorder.update.cart'))
